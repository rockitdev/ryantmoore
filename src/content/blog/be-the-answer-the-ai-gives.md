...
One layer deeper. The familiar ranking layer is still there. AI search adds retrieval and synthesis on top: it finds supporting pages, then writes an answer across them. Your pages may now be summarized or combined with other sources before a customer ever visits them.

Two consequences worth naming. Measurement becomes less complete, because an answer with no click and no referrer breaks the rank-to-click-to-analytics loop, so you fall back to fuzzier signals like brand mentions and direct traffic. A new failure mode appears alongside ranking low: the model can say something wrong about you. You cannot fully control that. You can only be the clearest, most consistent source, so there is less to get wrong.
